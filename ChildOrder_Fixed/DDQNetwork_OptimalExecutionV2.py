...
class Agent():
    ...
    def choose_action(self, observation, test=False):
        inventory = int(np.round(observation[1]))
        lowerbound = max(-5, -10 - inventory) + 5
        upperbound = min(5, 10 - inventory) + 5
        if (np.random.random() > self.epsilon or test == True):
            ...
            action = np.random.choice([i for i in range(lowerbound, upperbound + 1)])
        return action
    ...
    def learn(self):
        ...
        n = 0 
        q_eval_actions = q_eval.detach().numpy()
        for observation in states_:
            inventory = int(np.round(observation[1]))
            lowerbound = max(-5, -10 - inventory) + 5
            upperbound = min(5, 10 - inventory) + 5
            actions = q_eval_actions[n]
                #state = T.tensor([observation], dtype=T.float).to(self.q_eval.device)
                #actions = self.q_eval.forward(state).detach().numpy()[0]
            for i in range(lowerbound, upperbound + 1):
                if i == lowerbound:
    ...
    def save_model(self):
        T.save(self.q_eval.state_dict(), "Q_table_eval.pth")

    def load_model(self, path=None):
        if path == None:
            path = "Q_table_eval.pth"
        self.q_eval.load_state_dict(T.load(path))

    def test(self, env, num_games=100):
        pnls = []
        self.load_model
        for i in range(num_games):
            ...
            while not done:
                ...
                observation_, reward, done, cashflow = env.step(action)
                pnl += cashflow
        ...
        print('Mean of PnLs: ', avg_pnl)

        return np.array(pnls)

    def train(self, env, n_games=100000):
        scores, eps_history = [], []
        ...
        return scores, avg_score, eps_history
