...
def get_feedback(s, T, q, a):
    ...
    t = 1
    while t < dT + 1:
        cashflow_, reward_, q, s = SimMRStep(s, q, x, kappa, theta, sigma, dt, phi)
        period_reward += reward_
        period_cashflow += cashflow_
        t += 1
    return s_grid[abs(s - np.array(s_grid)).argmin()], q0 + a, period_reward, period_cashflow
...
def get_last_feedback(s, T, q, a):
    ...
    while t < dT + 1:
        ...
        period_cashflow += cashflow_
        t += 1
    cashflow_, reward_, q_, s_ = SimMRStep(s, q, 0, kappa, theta, sigma, dt, phi)
    #terminal_reward = q * (s_ - s) - c * phi * np.square(q)
    terminal_reward = - c * phi * np.square(q)
    period_reward += terminal_reward
    period_cashflow += q* s_ - phi*np.square(q)
...
def q_learning():
    ...
    for episode in range(int(niter)):
        ...
        while T < NT - 1:
            ...
            s_, q_, r, cash = get_feedback(s, T, q, a)  # get next state and the current reward
        ...
        s_, q_, r, cash = get_last_feedback(s, T, q, a)
        ...
        # s = s_
        # q = q_
    return q_table, s_matrix, q_matrix, a_matrix, r_matrix, pnls

def get_optimal_actions(T, q_table):
...
def plot_actions(T, q_table):
    # Get a data set of optimal actions for pairs (inventory,price)
    # At a fixed time
    # Generate sample data
    # Columns are x axis (inventory) and rows are y axis (price)

    yticklabels = np.round(np.arange(s_min, s_max, ds),3)
    cmap = sns.diverging_palette(20, 240, as_cmap=True)  # husl color system
    heat_map = sns.heatmap(get_optimal_actions(T, q_table), cmap=cmap, yticklabels=yticklabels)
    plt.xlabel("Inventory")
    plt.ylabel("Price")
    plt.title('Time: ' + str(T) +' Optimal Actions')
    plt.show()
