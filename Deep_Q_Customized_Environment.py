...
class env():
    def __init__(self,state_space,action_space):
        ...
        self.adms_action_dict = {q: self.adms_actions(q) for q in self.state_space.q}
        self.adms_action_index_dict = {q: self.adms_actions_indeces(q) for q in self.state_space.q}
    def reset(self):
...
class Agent:
    ...
    def choose_action(self, observation):
        ...
        if np.random.random() > self.epsilon:
            ...
            adms_actions_values = actions_values[:,self.env.adms_action_index_dict[observation[2]]]
            action_index = T.argmax(adms_actions_values).item()
            action = self.env.adms_action_dict[observation[2]][action_index]
        else:
            action = np.random.choice(self.env.adms_action_dict[observation[2]])
        return action
    ...
    def learn(self):
        ...
        for i in range(self.batch_size):
            adms_actions_values = q_eval[i,self.env.adms_action_index_dict[states_[i,2].item()]]
            action_index = T.argmax(adms_actions_values).item()
            action = self.env.adms_action_dict[states_[i,2].item()][action_index]
...
if __name__ == '__main__':
    agent = Agent(Env, gamma=0.99, epsilon=1.0, lr=0.001, n_actions=11, input_dims=[3], mem_size=10000,
                  batch_size=32, eps_min=0.1, eps_dec=5e-5, replace=100, fc1_dims=128,
                  fc2_dims=128)
    scores, eps_history = [], []
    n_games = 2000
    Q_table = np.zeros([len(s_grid), NT+1, len(q_grid), len(a_grid)])
    ...
    for t in range(NT):
        for j in range(len(q_grid)):
            for a in range(len(a_grid)):
                state = T.tensor([np.array([s_grid[i],t, q_grid[j]])], dtype=T.float)    
                Q_table[i,t,j,a] = QNN.forward(state)[0][a]
...
def get_optimal_actions(T):
    ...
    for i in range(np.shape(Q_table)[0]):
        for j in range(np.shape(Q_table)[2]):
            optimal_action_table[i, j] = Env.adms_actions(q_grid[j])[np.argmax(Q_table[i, T, j, Env.adms_actions_indeces(q_grid[j])])]
    return pd.DataFrame(optimal_action_table, columns=q_grid, index=np.round(s_grid,2))
